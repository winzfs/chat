...
export function TalkPanel({ onOpenCompose, posts }: { posts: D1TalkPost[]; onOpenCompose: () => void }) {
  return (
    <>
      ...
      <section className="recommend-section" aria-label="추천 사용자">
        ...
        <div className="recommend-list">{recommendedUsers.map((user) => <RecommendCard key={user.id} user={user} />)}</div>
      </section>
...
function RecommendCard({ user }: { user: (typeof recommendedUsers)[number] }) {
