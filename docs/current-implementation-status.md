...
- 기존 유저의 저장된 지역값이 목록 밖 값이면 설정 화면에서 `지역 재선택 필요`로 표시하고 지역 선택 드롭다운은 빈 값으로 초기화함
...
- 토크/사람/채팅 목록의 프로필 모달에서 `채팅 시작` 버튼으로 바로 대화방을 열 수 있음
- 채팅방 메시지의 프로필 모달은 이미 같은 대화방 안이므로 프로필 확인만 제공함
